...
in the green and red color planes, but at this point, preserving unbiased image
registration is more important, and symmetric linear interpolation is the only
way to do it.

3. ImageMagick's
[`convert`](http://www.imagemagick.org/Usage/color_basics/#separate) splits the
interpolated raw image in separate color planes, labeling them with `-0` for
...
used to remap the red and blue planes with `dechrom radial`. The red plane is
also translated by a small amount. At this stage, the only remaining raw data
reside in one half of the green channel that was not interpolated. The red and
blue pixels have been spread and shifted around.

5. However, the resulting color planes still have the same arrangement as the
original EXR array. They are processed as a surrogate raw image by the
...
is the TCA-corrected version of raw data. This project's lens-correction job is
done at this point (perhaps not perfectly, but reasonably well). However, the
whole point of doing it was to enable proper debayering, which can from now on
be accomplished in a number of ways. SSDD is one such method, and its output is
shown on the right. Note how it fixes the zipper artifact in the upper part of
the latch handle and how it removes some of that ugly chroma noise without
degrading the level of detail.
